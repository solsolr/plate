...
def kNN_train(train_fname, K, nclass, nsample):
    size = (40, 40)  # 숫자 영상 크기
    train_img = cv2.imread(train_fname, cv2.IMREAD_GRAYSCALE)  # 학습 영상 적재(읽기)
    h, w = train_img.shape[:2]
    dy = h % size[1]// 2        # 좌우 여백 크기
    dx = w % size[0]// 2        # 상하 여백 크기
    train_img = train_img[dy:h-dy-1, dx:w-dx-1]             # 학습 영상 여백 제거
    cv2.threshold(train_img, 32, 255, cv2.THRESH_BINARY, train_img)        # 학습 영상 이진화!
...
# 번호판 영상 전처리
def preprocessing_plate(plate_img):                         # 전처리 수행 함수 구현
    ...
    return ret_img                                          # 모서리 제거 사각형

# 숫자 및 문자 객체 검색
def find_objects(sub_mat):                                  # 숫자 및 문자 객체 검색
    results = cv2.findContours(sub_mat, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    contours = results[0] if int(cv2.__version__[0]) >= 4 else results[1]       # 객체 외곽선 검출

    rois = [cv2.boundingRect(contour) for contour in contours]
    rois = [(x, y, w, h, w*h) for x,y,w,h in rois if w / h < 2.5]

    text_rois = [(x, y, x+w, y+h) for x, y, w, h, a in rois if 45 < x < 80 and a > 60]  # 문자 객체 잡음 기준
    num_rois  = [(x, y, w, h) for x, y, w, h, a in rois  if not(45 < x < 80) and a > 150]   # 숫자 객체 잡음 기준

    if text_rois:                         # 분리된 문자 영역 누적
        # pts= cv2.sort(np.array(text_rois), cv2.SORT_EVERY_COLUMN)  # 열단위 오름차순
        pts= np.sort(text_rois, axis=0)             # y 방향 정렬 / 세로 방향 정렬
...
# 문자 객체 분류 결과인 레이블값에 대응하는 문자를 배열로 저장
    texts  = "가나다라마거너더러머버서어저고노도로모보" \
             "소오조구누두루무부수우주아바사자바하허호"                        # 학습 문자 집합

    numbers = [find_number(cell) for cell in cells]                    # 숫자 객체 찾기
    datas = [place_middle(num, (40,40)) for num in numbers]            # 중심 배치
    datas = np.reshape(datas, (len(datas), -1))                        # 1행 데이터 변환

    idx = np.argsort(object_rois, axis=0).T[0]                         # x 좌표 정렬 인덱스
    ...
    resp1 = resp1.flatten().astype('int')           # 전개 및 정수 변환
